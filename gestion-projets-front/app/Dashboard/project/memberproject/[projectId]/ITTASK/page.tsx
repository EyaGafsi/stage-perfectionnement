...
function StatusBadge({ task, onUpdate, isOwner }: {
  task: TaskIT;
  onUpdate: (id: number, s: TaskStatus, delayReason?: string) => Promise<{ ok: boolean }>;
  isOwner: boolean;
}) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in border border-slate-100">
        ...
        <div className="p-5">
          ...
          <textarea
            autoFocus
            className="w-full text-sm p-3 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 min-h-[100px] resize-none shadow-sm"
            placeholder="Ex: Problème technique inattendu, dépendance bloquante..."
            value={delayReason}
        ...
        <div className="px-5 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-2"
          >
...
function SprintBlock({ sprint, myUserId, onUpdateTask, filteredTaskIds }: {
  ...
  onUpdateTask: (id: number, s: TaskStatus, delayReason?: string) => Promise<{ ok: boolean }>;
...
export default function MemberITTasksPage() {
  ...
  const handleUpdateTask = useCallback(async (taskId: number, status: TaskStatus, delayReason?: string): Promise<{ ok: boolean }> => {
    const token = getToken();
    try {
